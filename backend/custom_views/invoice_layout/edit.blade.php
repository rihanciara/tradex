...
@component('components.widget', ['class' => 'box-solid', 'title' => __('lang_v1.qr_code')])
  <div class="row">
    ...
    @endphp
    @if(\Modules\JerryUpdates\Utils\JerrySettings::get('jerry_invoice_upi') == '1')
    {{-- Jerry: UPI Payment fields --}}
    <div class="col-sm-3">
      <div class="form-group">
        {!! Form::label('upi_id', 'UPI ID') !!}
        {!! Form::text('common_settings[upi_id]', $invoice_layout->common_settings['upi_id'] ?? '', ['class' => 'form-control', 'placeholder' => 'yourupi@bank']) !!}
      </div>
    </div>
    <div class="col-sm-4">
      <div class="form-group">
        <div class="checkbox">
          <label>
            {!! Form::checkbox('qr_code_fields[]', 'upi_payment_qr', in_array('upi_payment_qr', $qr_code_fields), ['class' => 'input-icheck']) !!}
            UPI Payment QR
          </label>
        </div>
      </div>
