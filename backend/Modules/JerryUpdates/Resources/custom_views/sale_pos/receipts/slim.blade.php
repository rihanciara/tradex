        ...
        <div class="ticket">
            ...
            <table style="margin-top: 25px !important" class="border-bottom width-100 table-f-12 mb-10">
                ...
                <tbody>
                 ...
                 <!-- by thufail  
  <div class="flex-box mt-3" style="display: flex; justify-content: space-between;">
    {{-- Left Side: QR Code --}}
    @if(!empty($receipt_details->qr_code_text))
       <div class="width-50 text-left" style="flex: 1; margin-left: 30px;">
            <p><strong>Scan to Pay</strong></p>
           {{-- <img src="data:image/png;base64,{{ DNS2D::getBarcodePNG($receipt_details->qr_code_text, 'QRCODE') }}" alt="QR Code" /> --}}
           <img src="data:image/png;base64,{{ DNS2D::getBarcodePNG($receipt_details->qr_code_text, 'QRCODE', 2, 2) }}"alt="QR Code" />

      {{-- <img style="max-width: 200px;" src="data:image/png;base64,{{ DNS2D::getBarcodePNG($receipt_details->qr_code_text, 'QRCODE', 2, 2) }}" alt="QR Code" /> --}}

        </div>
    @else
        <div class="width-50" style="flex: 1;"></div> {{-- Empty if no QR --}}
    @endif

    {{-- Right Side: Totals (stacked vertically) --}}
    <div class="width-50" style="flex: 1; display: flex; flex-direction: column; justify-content: center;">
     ...
     -->    
     <div class="flex-box mt-3" style="display: flex; align-items: flex-start; justify-content: flex-start; width: 100%; font-size: 11px;">
    
    {{-- Left Side: QR Code --}}
    @if(!empty($receipt_details->qr_code_text))
        <div style="flex: 0 0 35%; text-align: left;">
            <p style="margin: 0 0 3px 0; font-weight: bold;">Scan to Pay</p>
            <img src="data:image/png;base64,{{ DNS2D::getBarcodePNG($receipt_details->qr_code_text, 'QRCODE', 3, 3) }}" alt="QR Code" style="max-width: 100px; height: auto;" />
        </div>
    @endif

    {{-- Right Side: Totals --}}
    <div style="flex: 0 0 60%; margin-left: 10px; display: flex; flex-direction: column; text-align: left;">
        
        @if(!empty($receipt_details->total_items_label))
            <div style="display: flex; justify-content: space-between;">
                <p style="margin: 0;">{!! $receipt_details->total_items_label !!}</p>
                <p style="margin: 0;">{{$receipt_details->total_items}}</p>
            </div>
        @endif

        <div style="display: flex; justify-content: space-between; font-weight: bold;">
            <p style="margin: 0;">{!! $receipt_details->subtotal_label !!}</p>
            <p style="margin: 0;">{{$receipt_details->subtotal}}</p>
        </div>

        @if(!empty($receipt_details->discount))
            <div style="display: flex; justify-content: space-between;">
                <p style="margin: 0;">{!! $receipt_details->discount_label !!}</p>
                <p style="margin: 0;">(-) {{$receipt_details->discount}}</p>
            </div>
        @endif

        @if(!empty($receipt_details->total_line_discount))
            <div style="display: flex; justify-content: space-between;">
                <p style="margin: 0;">{!! $receipt_details->line_discount_label !!}</p>
                <p style="margin: 0;">(-) {{$receipt_details->total_line_discount}}</p>
            </div>
        @endif

        <div style="display: flex; justify-content: space-between; font-weight: bold; border-top: 1px dashed #000; margin-top: 5px; padding-top: 3px;">
            <p style="margin: 0;">{!! $receipt_details->total_label !!}</p>
            <p style="margin: 0;">{{$receipt_details->total}}</p>
        </div>

        @if(!empty($receipt_details->total_quantity_label))
            <div style="display: flex; justify-content: space-between; margin-top: 3px;">
                <p style="margin: 0;">{!! $receipt_details->total_quantity_label !!}</p>
                <p style="margin: 0;">{{$receipt_details->total_quantity}}</p>
            </div>
        @endif
