...
<head>
    ...
    <title>Expense Monthly Report - {{ $year ?? date('Y') }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="print-container">
        ...
        <div class="report-header">
            ...
            <div class="report-title">Expense Monthly Report</div>
            <div class="report-period">
                ...
                @endif
                @if(!empty($filters['created_by_name']))
                | Created By: {{ $filters['created_by_name'] }}
                @endif
                @endif
            </div>
        ...
        <div class="summary-section">
            <div class="summary-card primary">
                ...
                <div class="summary-value">{{ number_format($summary['total_categories'] ?? 0) }}</div>
            </div>
            ...
            <div class="summary-card danger">
                ...
                <div class="summary-value">${{ number_format($summary['total_expense'] ?? 0, 2) }}</div>
            </div>
            <div class="summary-card warning">
                <div class="summary-label">Avg per Category</div>
                <div class="summary-value">${{ number_format($summary['average_per_category'] ?? 0, 2) }}</div>
            </div>
        </div>
        @endif
        ...
        <div class="table-container">
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    $month) {
                    $monthTotals[$month] += $expense[$month] ?? 0;
                    }
                    @endphp
                    <tr>
                        <td>{{ $expense['category_name'] ?? 'No Category' }}</td>
                        <td class="currency">${{ number_format($expense['jan'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['feb'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['mar'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['apr'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['may'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['jun'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['jul'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['aug'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['sep'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['oct'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['nov'] ?? 0, 2) }}</td>
                        <td class="currency">${{ number_format($expense['dec'] ?? 0, 2) }}</td>
                        <td class="currency negative">
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="currency negative">
                            <strong>${{ number_format($grandTotalExpenses, 2) }}</strong>
                        </td>
                    </tr>
                </tfoot>
            </table>
