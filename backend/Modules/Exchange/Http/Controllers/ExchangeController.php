...
/**
 * FIXED: Handle exchange payments with discount-based refund approach
 * This eliminates negative payments and keeps all totals positive by jerry
...
private function handleExchangePayments($exchange_transaction, $exchange_difference, $original_amount, $new_amount, $request, $business_id, $user_id, $exchange_ref_no, $original_transaction)
{
    try {
        ...
        if ($exchange_difference > 0) {
            // SCENARIO 1: Customer pays extra (new items cost more)
            // Example: Return ₹200 item + Buy ₹400 items = Customer pays ₹200 extra
            
            \Log::info('Customer pays extra: ' . $exchange_difference);

            // Record only the NET amount customer needs to pay
            $payment = new \App\TransactionPayment();
            $payment->transaction_id = $exchange_transaction->id;
            $payment->business_id = $business_id;
            $payment->amount = $exchange_difference; // Only the difference
            $payment->method = $payment_method;
            $payment->paid_on = $current_date;
            $payment->created_by = $user_id;
            $payment->is_return = 0;
            $payment->paid_through_link = 0;
            $payment->is_advance = 0;
            $payment->payment_for = $exchange_transaction->contact_id ?? 1;
            $payment->note = sprintf(
                'Exchange payment - Original: %s, New: %s, Customer pays: %s',
                number_format($original_amount, 2),
                number_format($new_amount, 2),
                number_format($exchange_difference, 2)
            );

            $this->addPaymentDetails($payment, $request, $payment_method);
            ...
            // Update transaction to show original item credit as discount
            $exchange_transaction->discount_type = 'fixed';
            $exchange_transaction->discount_amount = $original_amount;
            $exchange_transaction->total_before_tax = $new_amount;
            $exchange_transaction->tax_amount = 0;
            $exchange_transaction->final_total = $new_amount - $original_amount; // Net amount
            $exchange_transaction->save();

            // Cash register entry - customer pays extra
            ...
            // SCENARIO 2: Customer gets refund (FIXED - Use discount approach)
            // Example: Return ₹200 item + Buy ₹125 item = Customer gets ₹75 refund
            
            $refund_amount = abs($exchange_difference);
            \Log::info('Customer gets refund (as discount): ' . $refund_amount);

            // FIXED: Apply the refund as a DISCOUNT, not a negative payment
            // This keeps all amounts positive and maintains accounting integrity
            
            // The new item's price becomes: new_amount - (discount = original_amount)
            // If original > new, discount exceeds new price, resulting in negative final_total
            // So we set final_total to 0 and note the excess as actual refund due
            
            $actual_refund_due = max(0, $original_amount - $new_amount);
            $final_price = max(0, $new_amount - $original_amount);

            // Update transaction with discount approach
            $exchange_transaction->discount_type = 'fixed';
            $exchange_transaction->discount_amount = $original_amount; // Full credit for returned item
            $exchange_transaction->total_before_tax = $new_amount;
            $exchange_transaction->tax_amount = 0;
            $exchange_transaction->final_total = 0; // Fully covered by exchange credit
            $exchange_transaction->additional_notes = sprintf(
                '%s | Exchange Credit Applied: %s | Item Price: %s | Net: %s (Fully covered by exchange)',
                $exchange_transaction->additional_notes ?? '',
                number_format($original_amount, 2),
                number_format($new_amount, 2),
            ...
            // Create a ZERO payment record (transaction is fully covered by discount)
            $payment = new \App\TransactionPayment();
            $payment->transaction_id = $exchange_transaction->id;
            $payment->business_id = $business_id;
            $payment->amount = 0; // No actual payment - fully covered
            $payment->method = $payment_method;
            $payment->paid_on = $current_date;
            $payment->created_by = $user_id;
            $payment->is_return = 0;
            ...
            $payment->payment_for = $exchange_transaction->contact_id ?? 1;
            $payment->note = sprintf(
                'Exchange - Paid with exchange credit (Original: %s, New: %s, Credit: %s)',
                number_format($original_amount, 2),
                number_format($new_amount, 2),
            ...
            // NO cash register entry needed - no actual cash movement
            // The exchange credit covers the purchase completely
            ...
            // Apply full discount equal to original amount
            $exchange_transaction->discount_type = 'fixed';
            $exchange_transaction->discount_amount = $original_amount;
            $exchange_transaction->total_before_tax = $new_amount;
            $exchange_transaction->tax_amount = 0;
            $exchange_transaction->final_total = 0; // Fully covered
            $exchange_transaction->additional_notes = sprintf(
                '%s | Even Exchange | Original: %s | New: %s (Fully covered by exchange)',
                $exchange_transaction->additional_notes ?? '',
                number_format($original_amount, 2),
                number_format($new_amount, 2)
            );
            ...
            $payment->payment_for = $exchange_transaction->contact_id ?? 1;
            $payment->note = sprintf(
                'Even exchange - No payment required (Original: %s = New: %s)',
                number_format($original_amount, 2),
                number_format($new_amount, 2)
            );
            ...
            // No cash register entries - no net cash movement
        }
        ...
        // Add exchange reference to original transaction
        $original_note = sprintf(
            'Items exchanged | Exchange Ref: %s | Date: %s | New Invoice: %s | %s',
            $exchange_ref_no,
            date('Y-m-d H:i:s'),
            $exchange_transaction->invoice_no,
            $exchange_difference > 0
                ? 'Customer paid: ' . number_format($exchange_difference, 2)
                : ($exchange_difference < 0
                    ? 'Exchange credit given: ' . number_format(abs($exchange_difference), 2)
                    : 'Even exchange')
        );
...
/**
 * FIXED: Handle exchange payments with discount-based refund approach
 * Final clean version - No logs, no notes anywhere
 */
private function handleExchangePayments($exchange_transaction, $exchange_difference, $original_amount, $new_amount, $request, $business_id, $user_id, $exchange_ref_no, $original_transaction)
{
    try {
        ...
        if ($exchange_difference > 0) {
            ...
            $payment->transaction_id = $exchange_transaction->id;
            $payment->business_id = $business_id;
            $payment->amount = $exchange_difference;
            $payment->method = $payment_method;
            $payment->paid_on = $current_date;
            $payment->created_by = $user_id;
            $payment->is_return = 0;
            ...
            $exchange_transaction->total_before_tax = $new_amount;
            $exchange_transaction->tax_amount = 0;
            $exchange_transaction->final_total = $new_amount - $original_amount;
            $exchange_transaction->save();
            ...
            // SCENARIO 2: Customer gets refund
            $exchange_transaction->discount_type = 'fixed';
            $exchange_transaction->discount_amount = $original_amount;
            $exchange_transaction->total_before_tax = $new_amount;
            $exchange_transaction->tax_amount = 0;
            $exchange_transaction->final_total = 0;
            $exchange_transaction->save();
  ...
  /**
 * Get exchange list via DataTables - FIXED by jerry claude
 */
public function getExchanges()
{
    ...
    if (request()->has('date_range') && request()->date_range != '') {
        ...
        if (count($date_range) == 2) {
            try {
                $start_date = \Carbon\Carbon::createFromFormat('m/d/Y', trim($date_range[0]))->startOfDay();
                $end_date = \Carbon\Carbon::createFromFormat('m/d/Y', trim($date_range[1]))->endOfDay();
                $exchanges->whereBetween('transaction_exchanges.exchange_date', [$start_date, $end_date]);
            } catch (\Exception $e) {
    ...
    $exchanges = $exchanges->select([
        ...
        'transaction_exchanges.original_amount',
        'transaction_exchanges.new_amount',
        'transaction_exchanges.exchange_difference',
    ...
    return DataTables::of($exchanges)
        ...
        ->editColumn('total_exchange_amount', function ($row) {
            return '<span class="display_currency" data-currency_symbol="true">' . number_format($row->total_exchange_amount, 2) . '</span>';
        })
        ...
        ->addColumn('created_by', function ($row) {
            return trim($row->created_by_name) ?: 'N/A';
        })
