...
<head>
    ...
    @yield('css')
    <link rel="stylesheet" href="{{ asset('/css/low-end-pc.css?v=' . $asset_v) }}">
...
<body
    ...
    <div class="tw-flex thetop">
        <script type="text/javascript">
            ...
            if (localStorage.getItem("upos_low_end_pc") == 'true') {
                var body = document.getElementsByTagName("body")[0];
                body.className += " low-end-pc";
            }
        </script>
        ...
        <script>
            $(document).ready(function() {
                if (localStorage.getItem("upos_low_end_pc") == 'true') {
                    ...
                    $('#toggleLowEndPc').prop('checked', true);
                    $('#btnLowEndPcUI').addClass('tw-text-yellow-400');
                }

                $('#toggleLowEndPc').change(function() {
                    var isLowEnd = $(this).is(':checked');
                    if (!isLowEnd) {
                        localStorage.setItem("upos_low_end_pc", 'false');
                        $('body').removeClass('low-end-pc');
                        if (typeof $ !== 'undefined' && $.fx) $.fx.off = false;
                        $('#btnLowEndPcUI').removeClass('tw-text-yellow-400');
                        toastr.success("Low-End PC Mode Disabled");
                    } else {
                        localStorage.setItem("upos_low_end_pc", 'true');
                        $('body').addClass('low-end-pc');
                        if (typeof $ !== 'undefined' && $.fx) $.fx.off = true;
                        $('#btnLowEndPcUI').addClass('tw-text-yellow-400');
                        toastr.success("Low-End PC Mode Enabled");
                    }
                });
            });
        </script>
