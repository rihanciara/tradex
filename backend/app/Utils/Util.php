...
class Util
{
    ...
    {
        $enabled_modules = session()->has('business') ? (session('business')['enabled_modules'] ?? null) : null;
        ...
        if (is_string($enabled_modules) && $enabled_modules != 'null') {
    ...
    public function activityLog($on, $action = null, $before = null, $properties = [], $log_changes = true, $business_id = null)
    {
        ...
        $timezone = is_array($business) ? ($business['time_zone'] ?? config('app.timezone')) : optional($business)->time_zone;
        if (!empty($timezone)) {
