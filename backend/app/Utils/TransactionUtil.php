...
class TransactionUtil extends Util
{
    ...
    {
        $status = DB::transaction(function () use ($transaction_id, $final_amount) {
            $transaction = Transaction::where('id', $transaction_id)->lockForUpdate()->firstOrFail();
            $total_paid = TransactionPayment::where('transaction_id', $transaction_id)
                ->lockForUpdate()
                ->select(DB::raw('COALESCE(SUM(IF(is_return = 0, amount, amount * -1)), 0) as total_paid'))
                ->first()
                ->total_paid;

            $current_final_amount = is_null($final_amount) ? $transaction->final_total : $final_amount;
            $status = 'due';
            if ($this->compareAmount($total_paid, $current_final_amount) >= 0) {
                $status = 'paid';
            } elseif ($this->compareAmount($total_paid, 0) > 0) {
                $status = 'partial';
            }
    ...
    public function updateContactBalance($contact, $amount, $type = 'add')
    {
        ...
        $operator = $type == 'deduct' ? '-' : '+';

        Contact::where('id', $contact_id)
            ->update(['balance' => DB::raw("ROUND(COALESCE(balance, 0) {$operator} {$amount}, 4)")]);
    }

    private function compareAmount($left, $right)
    {
        if (extension_loaded('bcmath')) {
            return bccomp((string) $left, (string) $right, 4);
        }
