...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // Globally force HTTPS for production domains or secure reverse-proxies to prevent POST dropouts
        if (str_contains(config('app.url'), 'https') || str_contains(request()->getHost(), 'dookanwale.com') || (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on') || (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] === 'https')) {
        ...
        View::composer(
            ['*'],
            function ($view) {
                $enabled_modules_raw = data_get(session('business'), 'enabled_modules');
                $enabled_modules = !empty($enabled_modules_raw) ? $enabled_modules_raw : [];
                
                // Defensive: if enabled_modules is a string (double-encoded JSON), decode it
