            ...
            return DataTables::of($query)
                    ...
                    ->addColumn('transaction_number', function ($row) {
                        ...
                        if ($row->type == 'sell') {
                            ...
                            $html = '<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-info btn-modal"
                                    data-href="'.action([\App\Http\Controllers\ExpenseController::class, 'show'], [$row->transaction_id]).'" data-container=".view_modal">'.$row->ref_no.'</button>';
                        } elseif ($row->type == 'sell_return') {
                            $html = '<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-info btn-modal"
                                    data-href="'.action([\App\Http\Controllers\SellReturnController::class, 'show'], [$row->transaction_id]).'" data-container=".view_modal">'.$row->invoice_no.'</button>';
                        } elseif ($row->type == 'purchase_return') {
                            $html = '<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-info btn-modal"
                                    data-href="'.action([\App\Http\Controllers\PurchaseReturnController::class, 'show'], [$row->transaction_id]).'" data-container=".view_modal">'.$row->ref_no.'</button>';
                        }
                    ...
                    ->editColumn('type', function ($row) {
                        ...
                        if ($row->type == 'sell') {
                            ...
                            $type = __('lang_v1.sell_return');
                        } elseif ($row->type == 'purchase_return') {
                            $type = __('lang_v1.purchase_return');
    ...
    public function postLinkAccount(Request $request)
    {
        ...
        try {
            ...
            if (request()->ajax()) {
                ...
                DB::transaction(function () use ($business_id, $payment_id, $account_id) {
                    ...
                    if (! empty($account_id)) {
                        Account::where('business_id', $business_id)->findOrFail($account_id);
                    ...
                    if (empty($payment_type)) {
                        $child_payment = TransactionPayment::with(['transaction'])
