...
class AccountController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $has_account_type_id = Schema::hasColumn('accounts', 'account_type_id');
            $has_account_types_table = Schema::hasTable('account_types');

            $account_select = ['accounts.name', 'accounts.account_number', 'accounts.note', 'accounts.id',
                'is_closed', DB::raw("SUM( IF(AT.type='credit', amount, -1*amount) ) as balance"),
                DB::raw("CONCAT(COALESCE(u.surname, ''),' ',COALESCE(u.first_name, ''),' ',COALESCE(u.last_name,'')) as added_by"),
            ];

            if ($has_account_type_id) {
                $account_select[] = 'accounts.account_type_id';
            } else {
                $account_select[] = DB::raw('NULL as account_type_id');
            }

            if ($has_account_type_id && $has_account_types_table) {
                $account_select[] = 'ats.name as account_type_name';
                $account_select[] = 'pat.name as parent_account_type_name';
            } else {
                $account_select[] = DB::raw('NULL as account_type_name');
                $account_select[] = DB::raw('NULL as parent_account_type_name');
            }

            if (Schema::hasColumn('accounts', 'account_details')) {
                $account_select[] = 'accounts.account_details';
            } else {
                $account_select[] = DB::raw('NULL as account_details');
            }
                                ...
                                $current_bal = AccountTransaction::where('account_id',
                                                ...
                                                ->where(function ($q) use ($row) {
                                                    $q->where('operation_date', '<', $row->operation_date)
                                                        ->orWhere(function ($sub) use ($row) {
                                                            $sub->where('operation_date', '=', $row->operation_date)
                                                                ->where('id', '<=', $row->id);
                                                        });
                                                })
    ...
    public function destroyAccountTransaction($id)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                $account_transaction = AccountTransaction::join('accounts as a', 'account_transactions.account_id', '=', 'a.id')
                    ->where('a.business_id', $business_id)
                ...
                if (in_array($account_transaction->sub_type, ['fund_transfer', 'deposit'])) {
                    ...
                    if (! empty($account_transaction->transfer_transaction_id)) {
                        $transfer_transaction = AccountTransaction::join('accounts as a', 'account_transactions.account_id', '=', 'a.id')
                            ->where('a.business_id', $business_id)
    ...
    public function postFundTransfer(Request $request)
    {
        ...
        try {
            ...
            if (! empty($amount) && (float) $amount > 0) {
                if ($from == $to) {
                    throw new \Exception('From and To accounts must be different.');
                }

                DB::beginTransaction();

                $locked_accounts = Account::where('business_id', $business_id)->whereIn('id', [$from, $to])->NotClosed()->lockForUpdate()->get();
                if ($locked_accounts->count() !== 2) {
        ...
        try {
            ...
            if (! empty($amount) && (float) $amount > 0) {
                ...
                if (! empty($from_account)) {
                    if ((int) $from_account === (int) $account_id) {
                        throw new \Exception('From and To accounts must be different.');
                    }

                    $locked_accounts = Account::where('business_id', $business_id)->whereIn('id', [$account_id, $from_account])->NotClosed()->lockForUpdate()->get();
                    if ($locked_accounts->count() !== 2) {
                        throw new \Exception(__('messages.something_went_wrong'));
                    }
        ...
        if (! empty($row->sub_type)) {
            ...
            if (! empty($row->transaction->type)) {
                if ($row->transaction->type == 'purchase') {
                    $supplier = ! empty($row->transaction->contact) ? $row->transaction->contact->full_name_with_business : __('lang_v1.unknown');
                    $details = __('lang_v1.purchase').'<br><b>'.__('purchase.supplier').':</b> '.$supplier.'<br><b>'.
                    ...
                    $details = __('lang_v1.expense').'<br><b>'.__('purchase.ref_no').':</b> <a href="#" data-href="'.action([\App\Http\Controllers\ExpenseController::class, 'show'], [$row->transaction->id]).'" class="btn-modal" data-container=".view_modal">'.$row->transaction->ref_no.'</a>';
                } elseif ($row->transaction->type == 'sell') {
                    $is_return = $row->is_return == 1 ? ' ('.__('lang_v1.change_return').')' : '';
                    $customer = ! empty($row->transaction->contact) ? $row->transaction->contact->full_name_with_business : __('lang_v1.unknown');
                    $details = __('sale.sale').$is_return.'<br><b>'.__('contact.customer').':</b> '.$customer.'<br><b>'.
                    ...
                    $customer = ! empty($row->transaction->contact) ? $row->transaction->contact->full_name_with_business : __('lang_v1.unknown');
                    $details = __('lang_v1.sell_return').'<br><b>'.__('contact.customer').':</b> '.$customer.'<br><b>'.
                    __('sale.invoice_no').':</b> <a href="#" data-href="'.action([\App\Http\Controllers\SellReturnController::class, 'show'], [$row->transaction->id]).'" class="btn-modal" data-container=".view_modal">'.$row->transaction->invoice_no.'</a>';
                } elseif ($row->transaction->type == 'purchase_return') {
                    $supplier = ! empty($row->transaction->contact) ? $row->transaction->contact->full_name_with_business : __('lang_v1.unknown');
                    $details = __('lang_v1.purchase_return').'<br><b>'.__('purchase.supplier').':</b> '.$supplier.'<br><b>'.
                    __('purchase.ref_no').':</b> <a href="#" data-href="'.action([\App\Http\Controllers\PurchaseReturnController::class, 'show'], [$row->transaction->id]).'" class="btn-modal" data-container=".view_modal">'.$row->transaction->ref_no.'</a>';
                }
            } else {
    ...
    public function updateAccountTransaction(Request $request, $id)
    {
        ...
        try {
            ...
            $account_transaction = AccountTransaction::with(['transfer_transaction'])->lockForUpdate()->findOrFail($id);
            ...
            if (! empty($account_transaction->transfer_transaction)) {
                $transfer_transaction = AccountTransaction::lockForUpdate()->findOrFail($account_transaction->transfer_transaction->id);
