...
$(document).ready(function() {
    /*added by thufail */
  // ---------------- FULL PRODUCT CACHE ADDON ----------------
 window.fullProductCache = { data: [], ts: 0 };  
  var FULL_CACHE_TTL = 24 * 60 * 60 * 1000; // 24 hours

  // Load cache from localStorage
  try {
      var raw = localStorage.getItem('fullProductCache');
      if (raw) {
          fullProductCache = JSON.parse(raw) || { data: [], ts: 0 };
          if ((Date.now() - fullProductCache.ts) > FULL_CACHE_TTL) {
              fullProductCache = { data: [], ts: 0 };
          }
      }
  } catch(e) {
      fullProductCache = { data: [], ts: 0 };
      localStorage.removeItem('fullProductCache');
  }

  // Save cache helper
  function saveFullProductCache() {
      try {
          localStorage.setItem('fullProductCache', JSON.stringify(fullProductCache));
      } catch(e) {
          localStorage.removeItem('fullProductCache');
          fullProductCache = { data: [], ts: 0 };
      }
  ...
  function clearFullProductCache() {
      fullProductCache = { data: [], ts: 0 };
      try { localStorage.removeItem('fullProductCache'); } catch(e) {}
  }

  // Fetch full product dataset if not cached
  function preloadFullProductCache(callback) {
      if (fullProductCache.data.length > 0) {
          if (callback) callback(fullProductCache.data);
          return;
      }
      $.getJSON('/products/list', { not_for_selling: 0 }, function(data) {
          // ⚠ Keep full object (don’t strip fields) so POS works properly
  ...
  // On page load → preload cache
  preloadFullProductCache(function(data){
      console.log("Full product cache loaded (" + data.length + " items).");
  });
...
// Bind clear cache button (replace your old one)
...
// thufail ends
  ...
  /*  if ($('#search_product').length) {
        //Add Product
        //changed delay from 1000 to 250 by thufail
        $('#search_product')
            .autocomplete({
                delay: 250,
                ...
                response: function(event, ui) {
                    if (ui.content.length == 1) {
                       ...
                       // $('input#search_product').select();
                        //commented by thufail to remove auto selection
                    }
                },
    ...
    commented by thufail
    */
// ---------------- AUTOCOMPLETE ---------------- by thufail
if ($('#search_product').length) {
        ...
        source: function(request, response) {
            ...
            if (fullProductCache.data && fullProductCache.data.length > 0) {
                ...
                for (let i = 0; i < fullProductCache.data.length; i++) {
                    ...
                    // ✅ Text search
                    if (name === term || sku === term || sub_sku === term) {
                        exactMatches.push(p);
                    } else if (name.startsWith(term) || sku.startsWith(term) || sub_sku.startsWith(term)) {
                        startsWithMatches.push(p);
                    } else if (name.includes(term) || sku.includes(term) || sub_sku.includes(term)) {
    ...
    console.log("Loaded from cache:", filtered.length, "results for term:", term);
    response(filtered);
    return;
}
            ...
            response: function(event, ui) {
                ...
                if (/^\d+$/.test(term) && ui.content.length == 1) {
                    // ✅ Auto-select for numeric searches (barcode / ID)
                    ui.item = ui.content[0];
                ...
                // ✅ For text search with 1 match: just show the dropdown, no auto-select
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success) {
                    ...
                    $(".pos_product_div").animate({ scrollTop: $('.pos_product_div').prop("scrollHeight")}, 250);
                } else {
                    toastr.error(result.msg);
                    $('input#search_product')
                        .focus()
